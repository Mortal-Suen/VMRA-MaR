...
from vmramar.utils import parsing
from vmramar.utils.generic import md5
...
RESULT_KEY_STEMS = [
    '{}_loss', '{}_reg_loss', '{}_accuracy', '{}_auc', '{}_precision', 
    '{}_recall', '{}_f1', '{}_c_index', '{}_decile_recall',
    '{}_asymmetry_loss', '{}_temporal_loss'  
]
RESULT_KEY_STEMS += ['{}_'+'{}year_auc'.format(i) for i in range(1,10)]

ADDITIONAL_RESULT_KEYS = [
    'dev_auc', 'dev_tnr_by_threshold', 'dev_fnr_by_threshold', 
    'test_fnr_by_threshold', 'test_tnr_by_threshold',
    'dev_asymmetry_score', 'test_asymmetry_score'  
]

LOG_KEYS = ['results_path', 'model_path', 'log_path']
SORT_KEY = 'dev_loss'

parser = argparse.ArgumentParser(description='VMRA-MaR Grid Search Dispatcher')
...
parser.add_argument('--result_path', type=str, default="results/grid_search.csv", help="path to store grid_search table")
...
def launch_experiment(gpu, flag_string):
    '''Launch an experiment with VMRA-MaR specific configurations'''
    ...
    # Construct experiment command with VMRA-MaR specific flags
    experiment_string = "CUDA_VISIBLE_DEVICES={} python -u scripts/main.py {} --results_path {}".format(
        gpu, flag_string, results_path)

    pipe_str = ">>" if "--resume" in flag_string and not args.rerun_experiments else ">"
...
def update_sumary_with_results(result_path, log_path, experiment_axies, summary):
    '''Update summary with VMRA-MaR specific metrics'''
    ...
    # Process all result keys including VMRA-MaR specific ones
    ...
    for key in summary_columns:
        summary_dict[key] = result_dict.get(key, 'NA')
